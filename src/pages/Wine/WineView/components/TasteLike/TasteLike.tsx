...
export function TasteLike(props: Props): JSX.Element {
  const user = useSelector<MainState, any>((state: MainState) => state.authModule.user);
  ...
  return (
    <>
      ...
      <TastePreview wine={props.wine || ({} as Wine)} query={taste || ({} as WineQuery)} onClose={setTaste} />
    </>
  );
}
