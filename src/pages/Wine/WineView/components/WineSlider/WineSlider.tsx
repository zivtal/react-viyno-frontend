...
export const WineSlider = (props: Props): JSX.Element | null => {
  ...
  const data: Array<Wine> = !props.loading ? props.wines || [] : Array(props.repeat || 8).fill(WINE_DEMO);

  const sliderStyle = () => {
    const sec = 2;
    const pos = position ? -((Math.min((position + 1) * itemPerPage, data?.length || 8) / itemPerPage) * 100 - 100) : 0;
    return {
  ...
  const nextEnabled = () => {
    return rtl ? position : (data?.length || 8) > position * itemPerPage + itemPerPage;
  };

  const backEnabled = () => (rtl ? (data?.length || 8) > position * itemPerPage + itemPerPage : position);
  ...
  return loading || data?.length ? (
    <>
      ...
      <div
        className={`wine-slider-container ${isFlexMode ? 'flex-mode-enabled' : ''}`}
        // onTouchStart={touchStart}
        onTouchMove={touchMove}
        onTouchEnd={touchEnd}
      >
        {nextEnabled() ? <button className="next" onClick={() => setPosition(rtl ? position - 1 : position + 1)}></button> : null}

        {backEnabled() ? <button className="back" onClick={() => setPosition(rtl ? position + 1 : position - 1)}></button> : null}
          ...
          <div className="wine-slider" style={sliderStyle()}>
            <Loader if={loading} type="overlay-skeleton">
              {data?.map((wine: Wine, index) => (
                <WineCardPreview wine={wine} style={{ width: `calc(${100 / itemPerPage}% - 20px)` }} isMinimal={true} key={index} />
              ))}
            </Loader>
          </div>
        </div>
