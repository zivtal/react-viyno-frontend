...
export default class ImageService {
  ...
  public static async resize(file: File, { megaPixel, quality, type, crop }: ImageResize): Promise<File> {
    ...
    const sw = crop && canvas.width > canvas.height ? this.calcCrop(canvas, crop.ratio) : canvas.width;
    const sh = crop && canvas.width < canvas.height ? this.calcCrop(canvas, crop.ratio) : canvas.height;
    ...
    return FileService.toFile(canvas.toDataURL(type || file.type, quality || 0.9), file);
  }

  public static async bulkResize(file: File, options: Array<BulkImageResize>): Promise<Array<File>> {
  ...
  private static calcCrop({ width, height }: ImageDimensions, crop: number): number {
