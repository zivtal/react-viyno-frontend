export default class StringService {
  ...
  public static toUniqueId(text: string): string {
    return [...text]
      .reduce(
        (sum: number, char: string, index: number): number =>
          sum + char.charCodeAt(0) ** ((index % 10) + 1),
        0
      )
      .toString(16);
  }

  public static toHslColor(
    value: string,
    s: number = 30,
    l: number = 80
  ): string {
  ...
  public static toKebabCase(
    value?: string,
    validCharsets: boolean = false
  ): string | null {
